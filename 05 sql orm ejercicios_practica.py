...
engine = sqlalchemy.create_engine("sqlite:///secundaria2.db")
...
def fill():
    ...
    tutor1 = Tutor(name="Juana De Arco")
    tutor2 = Tutor(name="Alejandro Magno")

    estudiante1 = Estudiante(name="Pedro Picapiedra", age=15, grade=1, tutor=tutor1)
    estudiante2 = Estudiante(name="Carolina Ardohain", age=16, grade=2, tutor=tutor2)
    estudiante3 = Estudiante(name="Marcelo Gallardo", age=15, grade=6, tutor=tutor1)
    estudiante4 = Estudiante(name="Sofia Maria", age=16, grade=5, tutor=tutor2)
    estudiante5 = Estudiante(name="Luis Miguel", age=15, grade=1, tutor=tutor1)

    session.add_all([estudiante1, estudiante2, estudiante3, estudiante4, estudiante5])
...
def search_by_tutor(tutor):
    ...
    query = session.query(Estudiante).filter(Estudiante.tutor.has(name=tutor)).join(Tutor)
...
def modify(id, name):
    ...
    # # Buscar el estudiante que se desea actualizar
    ...
    tutor_actualizado = query.first()

    # # Actualizar el tutor
    estudiante.tutor = tutor_actualizado

    # Actualizar la persona con nombre "name"
    #consulta = session.query(Estudiante).filter(Estudiante.name == name).update({Estudiante.id:Tutor.id})
...
def count_grade(grade):
    ...
    print('Estuadiantes de', grade, 'encontradas:', result)
